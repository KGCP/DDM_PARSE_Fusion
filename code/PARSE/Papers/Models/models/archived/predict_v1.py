...
def predict(pred_dataloader, model_path):
    ...
    for input_ids_list, input_mask_list, segment_ids_list, label_ids_list, tokens_list in tqdm(pred_dataloader):
        ...
        # accumulate current words
        for i in range(config.batch_size):
            real_len = int(input_mask[i].sum())
            cls_num = tokens_list[i].count("[CLS]")
            sep_num = tokens_list[i].count("[SEP]")
            unk_num = tokens_list[i].count("[UNK]")
            accu_words = accu_words + real_len - cls_num - sep_num - unk_num
    i=1
