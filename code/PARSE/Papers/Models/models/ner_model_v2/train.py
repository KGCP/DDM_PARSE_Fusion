...
def evaluate(model, val_dataloader):
    ...
    target_eval_names = set(config.tags)
...
def train(train_dataloader, valid_dataloader):
    ...
    logger.info(f'\rInitialized device {device}.')

    # initialize model
    logger.info('Initializing model...')
    if config.pretrained_model in ["BERT-base", "ROBERTA", "ALBERT", "OAG_BERT"]:
    ...
    if config.optimizer == "BertAdam":
        ...
        optimizer = SGD(optimizer_params, lr=config.general_learning_rate, weight_decay=config.weight_decay_rate)
    else:
    ...
    # save model
    if f1_val > 0.65 or config.is_save:
...
if __name__ == '__main__':
    ...
    if mode == "ssh":
        ...
        val_dataloader = DataLoader(config, data_file=config.processed_data + 'test_title_v3.csv',
                                    is_test=True, tokenizer=tokenizer, mode=mode)
    else:
        train_dataloader = DataLoader(config,
                                      data_file=config.processed_data + 'train_title_v3.csv',
                                      tokenizer=tokenizer, mode=mode)
        val_dataloader = DataLoader(config, data_file=config.processed_data + 'train_title_v3.csv',
                                    is_test=True, tokenizer=tokenizer, mode=mode)
