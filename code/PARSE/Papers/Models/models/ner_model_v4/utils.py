...
def load_data(data_file_name, min_seq_length):
    with open(data_file_name, 'r', encoding='utf-8') as f:
        ...
        words = ""
        labels = ""
        df = pd.read_csv(f, names=['words', 'labels'], sep=get_sep(data_file_name))
        cur_length = 0
...
if __name__ == "__main__":
    ...
    if last_sample is not None:
        print("Last sample:")
        text_ids_list, text_mask_list, segment_ids_list, label_ids_list, tokens_list = last_sample
        print(text_ids_list)
        print(text_mask_list)
