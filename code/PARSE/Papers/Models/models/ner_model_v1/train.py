...
import json
#TODO: add charCNN in this model
...
def train(train_dataloader, valid_dataloader):
    ...
    if config.pretrained_model in ["bert", "roberta", 'xlm_roberta', "deberta"]:
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(config.batch_size,
                                  data_file='../dataset/train0.csv',
                                  tokenizer=tokenizer, max_seq_length=config.max_sequence_length, pretrained_model=config.pretrained_model)
    val_dataloader = DataLoader(config.batch_size, data_file=config.processed_data + 'test-abs.data',
