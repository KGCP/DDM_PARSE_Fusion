...
def get_entity(words_list, label_list, result_dict):
    ...
    while i < len(words_list):
        if label_list[i] not in ["O", "[CLS]", "[UNK]", "[SEP]"]:
            if "-" not in label_list[i]:  # 检查是否存在"-"符号
                i += 1
                continue
            pointer = i
            entity = ""
            ...
            entity += words_list[i]
            word_cnt = 1
            while pointer + 1 < len(label_list):
                pointer += 1
                if label_list[pointer] in ["[CLS]", "[UNK]", "[SEP]", "O"]:
                    break
                if "-" not in label_list[pointer]:  # 检查是否存在"-"符号
                    break
...
def predict(pred_dataloader, model_path, file_name):
    # remove map_lcation
    ...
    for input_ids_list, input_mask_list, segment_ids_list, label_ids_list, tokens_list in tqdm(pred_dataloader):
        ...
        real_batch_tags = []
        loop_size = min(config.batch_size, input_mask.size(0))
    ...
    dict_path = os.path.join(config.predict_dict_output_path, file_name.replace('.csv', '_dict.json'))
    ...
    df = DataFrame(temp)
    text_path = os.path.join(config.predict_text_output_path, file_name.replace('.csv', '_predicted_text.csv'))
    # df.to_csv(text_path, index=False)

if __name__ == '__main__':
    config = Config(mode="ssh")
    if config.predict_model == "BERT-base":
        tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")
    elif config.predict_model == "ALBERT":
        from transformers import AlbertTokenizer

        tokenizer = AlbertTokenizer.from_pretrained('albert-base-v2')
    elif config.predict_model == "ROBERTA":
        from transformers import RobertaTokenizer

        tokenizer = RobertaTokenizer.from_pretrained('roberta-base')
    elif config.predict_model == "SciBERT":
        tokenizer = AutoTokenizer.from_pretrained('allenai/scibert_scivocab_uncased')
    elif config.predict_model == "LinkBERT":
        tokenizer = AutoTokenizer.from_pretrained('michiyasunaga/LinkBERT-base')
    else:
        raise Exception("pretrained model not defined")

    # Create a list to hold the processed files
    processed_files = []

    # Try to read the processed_files.csv, if it exists.
    try:
        with open("processed_files.csv", 'r') as file_record:
            reader = csv.reader(file_record)
            processed_files = list(reader)
    except FileNotFoundError:
        pass  # If the file does not exist, that's OK; we'll create it.

    # Now, convert the list of lists to a single list
    processed_files = [item for sublist in processed_files for item in sublist]

    # Process the files
    with open("processed_files.csv", 'a', newline='') as file_record:
        writer = csv.writer(file_record)
        for file in os.listdir(config.askg_cs_papaer_path):
            if file.endswith(".csv"):
                ...
                dev_dataloader = DataLoader(config.batch_size,
                                            data_file=os.path.join(config.askg_cs_papaer_path, file),
                                            tokenizer=tokenizer, is_test=True,
                                            max_seq_length=config.max_sequence_length,
                                            pretrained_model=config.pretrained_model)
                predict(dev_dataloader, config.predict_model_path, file)
                # Write the processed file name to csv
